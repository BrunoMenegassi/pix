...
bool CmdPostProcessingBeginDraw::Execute(const std::vector<string>& params)
{
	PostProcessing::Get()->BeginDraw();
	return true;
}

bool CmdPostProcessingEndDraw::Execute(const std::vector<string>& params)
{
	PostProcessing::Get()->EndDraw();
	return true;
}

bool CmdPostProcessingSetEffectType::Execute(const std::vector<string>& params)
{
	if (params.size() < 1)
	{
		return false;
